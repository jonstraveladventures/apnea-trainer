import { Phase, SessionTemplate } from '../types/index';
import { formatTime } from './trainingLogic';

// Helper function to add common phases (stretch confirmation, tidal breathing, cooldown)
export const addCommonPhases = (phases: Phase[], template: SessionTemplate, maxHoldSeconds: number): Phase[] => {
...
export const addCooldownPhase = (phases: Phase[]): Phase[] => {
  const newPhases = [...phases];
...
export const parseMaximalBreathHoldTraining = (template: SessionTemplate, maxHoldSeconds: number): Phase[] => {
  const phases: Phase[] = [];
...
// Unified CO₂ table parser — used by both 'Traditional CO₂ Tables' and 'CO₂ Tolerance'
// (previously two nearly identical functions)
export const parseCo2Table = (template: SessionTemplate, maxHoldSeconds: number, descriptionPrefix: string = 'CO₂'): Phase[] => {
  const phases: Phase[] = [];

  const holdCount = template.holdCount || 5;
  const holdStart = template.holdStartDuration || 45;
  const holdIncrease = template.holdIncrease || 15;
  const restDuration = template.restDuration || 45;

  for (let i = 0; i < holdCount; i++) {
    const holdTime = holdStart + (i * holdIncrease);
    phases.push({
      type: 'hold',
      duration: holdTime,
      description: `${descriptionPrefix} Hold ${i + 1}/${holdCount} (${formatTime(holdTime)})`
    });
    if (i < holdCount - 1) {
      phases.push({
        type: 'rest',
        duration: restDuration,
        description: `${descriptionPrefix} Rest ${i + 1}/${holdCount - 1} (${formatTime(restDuration)})`
      });
    }
  }
...
export const parseTraditionalCo2Tables = (template: SessionTemplate, maxHoldSeconds: number): Phase[] =>
  parseCo2Table(template, maxHoldSeconds, 'Traditional CO₂');

export const parseCo2Tolerance = (template: SessionTemplate, maxHoldSeconds: number): Phase[] =>
  parseCo2Table(template, maxHoldSeconds, 'CO₂');

export const parseBreathControl = (template: SessionTemplate, maxHoldSeconds: number): Phase[] => {
  const phases: Phase[] = [];
...
export const parseO2Tolerance = (template: SessionTemplate, maxHoldSeconds: number): Phase[] => {
  const phases: Phase[] = [];
...
export const parseMentalTechnique = (template: SessionTemplate, maxHoldSeconds: number): Phase[] => {
  const phases: Phase[] = [];
...
export const parseAdvancedCo2Table = (template: SessionTemplate, maxHoldSeconds: number): Phase[] => {
  const phases: Phase[] = [];
...
export const parseMaxBreathHold = (template: SessionTemplate, maxHoldSeconds: number): Phase[] => {
  const phases: Phase[] = [];
...
export const parseRecoveryFlexibility = (template: SessionTemplate, maxHoldSeconds: number): Phase[] => {
  const phases: Phase[] = [];
...
export const parseComfortableCo2Training = (template: SessionTemplate, maxHoldSeconds: number): Phase[] => {
  const phases: Phase[] = [];
...
export const parseSessionPhases = (sessionType: string, template: SessionTemplate, maxHoldSeconds: number): Phase[] => {
  let phases: Phase[] = [];

  // Add common phases (stretch confirmation, tidal breathing) at the BEGINNING for breath-hold sessions
  ...
  switch (sessionType) {
    case 'Maximal Breath-Hold Training':
      phases = [...phases, ...parseMaximalBreathHoldTraining(template, maxHoldSeconds)];
      break;
    case 'Traditional CO₂ Tables':
      phases = [...phases, ...parseTraditionalCo2Tables(template, maxHoldSeconds)];
      break;
    case 'CO₂ Tolerance':
      phases = [...phases, ...parseCo2Tolerance(template, maxHoldSeconds)];
      break;
    case 'Breath Control':
      phases = parseBreathControl(template, maxHoldSeconds);
      break;
    case 'O₂ Tolerance':
      phases = [...phases, ...parseO2Tolerance(template, maxHoldSeconds)];
      break;
    case 'Mental + Technique':
      phases = [...phases, ...parseMentalTechnique(template, maxHoldSeconds)];
      break;
    case 'Advanced CO₂ Table':
      phases = [...phases, ...parseAdvancedCo2Table(template, maxHoldSeconds)];
      break;
    case 'Max Breath-Hold':
      phases = [...phases, ...parseMaxBreathHold(template, maxHoldSeconds)];
      break;
    case 'Recovery & Flexibility':
      phases = [...phases, ...parseRecoveryFlexibility(template, maxHoldSeconds)];
      break;
    case 'Comfortable CO₂ Training':
      phases = [...phases, ...parseComfortableCo2Training(template, maxHoldSeconds)];
      break;
    default:
