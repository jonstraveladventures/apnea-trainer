...
const ExerciseInstructionsModal: React.FC<ExerciseInstructionsModalProps> = ({ isOpen, instruction, onClose }) => {
  return (
    <ModalShell
      isOpen={isOpen && !!instruction}
      onClose={onClose}
      labelledBy="exercise-instructions-modal-title"
      panelClassName="bg-white dark:bg-deep-800 rounded-lg p-6 max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto"
    >
      {instruction && (
        <>
          <div className="flex justify-between items-center mb-4">
            <h3 id="exercise-instructions-modal-title" className="text-xl font-semibold text-gray-900 dark:text-white">{instruction.title}</h3>
            <button
              onClick={onClose}
